...
// ═══════════════════════════════════════════════════════════════
// CONFIG
// ✅ FIXED: 'nextsign' → 'nexsign' (extra 't' সরানো হয়েছে)
// ═══════════════════════════════════════════════════════════════
const BASE = (
  import.meta.env.VITE_API_BASE_URL ||
...
// ═══════════════════════════════════════════════════════════════
// DOCUMENT API (Module 1 — untouched)
// ═══════════════════════════════════════════════════════════════
export const documentApi = {
  getAll:      (params = {}) => api.get('/documents',               { params }),
  getOne:      (id)          => api.get(`/documents/${id}`),
  getAudit:    (id)          => api.get(`/documents/${id}/audit`,   { noCache: true }),
  upload:      (formData)    => api.post('/documents/upload',        formData),
  uploadLogo:  (formData)    => api.post('/documents/upload-logo',   formData),
  update:      (id, data)    => api.put(`/documents/${id}`,          data),
  send:        (formData)    => api.post('/documents/upload-and-send', formData).then(res => {
    apiCache.invalidatePattern('/documents');
    return res;
  }),
  delete:      (id)          => api.delete(`/documents/${id}`).then(res => {
    apiCache.invalidatePattern('/documents');
    return res;
  }),
  validateToken:   (token) => api.get(`/documents/sign/validate/${token}`, { noCache: true }),
  submitSignature: (data)  => api.post('/documents/sign/submit', data),
  getPdfProxy:     (token) => `${BASE}/documents/sign/${token}/pdf`,
};

// ═══════════════════════════════════════════════════════════════
// TEMPLATE API (Module 2)
// ═══════════════════════════════════════════════════════════════
export const templateApi = {
  getAll: (params = {}) =>
    api.get('/templates', { params }),
  ...
  // ✅ PDF proxy URL builder — iframe এ এই URL দাও
  // Cloudinary URL সরাসরি iframe এ দিলে X-Frame-Options block করে
  // এই proxy endpoint সেটা bypass করে
...
// ═══════════════════════════════════════════════════════════════
if (typeof window !== 'undefined') {
  window.addEventListener('online',  () => { apiCache.clear(); });
  window.addEventListener('offline', () => {});
}
